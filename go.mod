module highway

go 1.12

require (
	github.com/ethereum/go-ethereum v1.9.5
	github.com/golang/protobuf v1.3.2
	github.com/googleapis/gax-go v2.0.2+incompatible // indirect
	github.com/incognitochain/incognito-chain v0.0.0-20191010075117-36229f4b5e5e
	github.com/libp2p/go-libp2p v0.3.1
	github.com/libp2p/go-libp2p-core v0.2.2
	github.com/libp2p/go-libp2p-crypto v0.1.0
	github.com/libp2p/go-libp2p-host v0.1.0
	github.com/libp2p/go-libp2p-peer v0.2.0
	github.com/incognitochain/go-libp2p-grpc v0.0.0-20181024123959-d1f24bf49b50
	github.com/libp2p/go-libp2p v0.3.1
	github.com/libp2p/go-libp2p-core v0.2.2
	github.com/libp2p/go-libp2p-host v0.1.0 // indirect
	github.com/libp2p/go-libp2p-net v0.1.0 // indirect
	github.com/libp2p/go-libp2p-peer v0.2.0
	github.com/libp2p/go-libp2p-protocol v0.1.0 // indirect
	github.com/libp2p/go-libp2p-pubsub v0.1.1
	github.com/libp2p/go-tcp-transport v0.1.0
	github.com/libp2p/go-ws-transport v0.1.0
	github.com/multiformats/go-multiaddr v0.0.4
	github.com/multiformats/go-multihash v0.0.5
	github.com/ninjadotorg/constant v0.0.0-20190311033852-a552b54dcc1a // indirect
	github.com/paralin/go-libp2p-grpc v0.0.0-20171228081709-3d5d33466aef
	github.com/pkg/errors v0.8.1
	github.com/stathat/consistent v1.0.0
	google.golang.org/grpc v1.20.1
	github.com/pkg/errors v0.8.1
	github.com/stathat/consistent v1.0.0
	google.golang.org/grpc v1.20.1
	stathat.com/c/consistent v1.0.0 // indirect
)
