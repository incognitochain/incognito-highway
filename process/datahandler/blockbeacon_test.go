...
func TestBlkBeaconHandler_HandleDataFromTopic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				topicReceived: "blockbeacon--aaaa-nodepub",
				dataReceived:  libp2p.Message{},
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := handler.HandleDataFromTopic(tt.args.topicReceived, tt.args.dataReceived); (err != nil) != tt.wantErr {
				t.Errorf("BlkBeaconHandler.HandleDataFromTopic() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
