...
var ConfigFromEnvVars LogConfig

// The configuration items in LogConfig are what is supplied by the user
...
//     - "LOG_TRACE_LEVEL=client.go=1,ip*=5,3"
...
//     - "LOG_LOG_LEVEL=client.go=ERROR,INFO,ip*=WARN"
...
func updateConfigFromFile(config *LogConfig) {
...
func getTimeFormat(config LogConfig) string {
...
func initialize(config LogConfig, reInitEnvVars bool) {
	var err error
...
func basicLog(logLevel int, traceLevel int, isLocked bool, format string, prefixAddition string, a ...interface{}) {
	...
	switch logLevel {
	case levelInfo:
		logLine = fmt.Sprintf("%s%c[1;32m%-7s%c[0m: %s%s", now.Format(settingDateTimeFormat), 0x1B, levelDecoration, 0x1B, callerInfo, msg)
		break
	case levelErr:
		logLine = fmt.Sprintf("%s%c[1;31m%-7s%c[0m: %s%s", now.Format(settingDateTimeFormat), 0x1B, levelDecoration, 0x1B, callerInfo, msg)
		break
	case levelWarn:
		logLine = fmt.Sprintf("%s%c[1;33m%-7s%c[0m: %s%s", now.Format(settingDateTimeFormat), 0x1B, levelDecoration, 0x1B, callerInfo, msg)
		break
	default:
		logLine = fmt.Sprintf("%s%-7s: %s%s", now.Format(settingDateTimeFormat), levelDecoration, callerInfo, msg)
	}
...
// parameter. The LOG_TRACE_LEVEL variable is used to determine which levels
// of trace message are output: Every message with a level lower or equal to
// what is specified in LOG_TRACE_LEVEL. If LOG_TRACE_LEVEL is not defined at
...
// Debug prints a message if LOG_LEVEL is set to DEBUG.
func Debug(a ...interface{}) {
	basicLog(levelDebug, notATrace, false, "", "", a...)
}

// Debugf prints a message if LOG_LEVEL is set to DEBUG, with formatting.
func Debugf(format string, a ...interface{}) {
	basicLog(levelDebug, notATrace, false, format, "", a...)
}

// Info prints a message if LOG_LEVEL is set to INFO or lower.
func Info(a ...interface{}) {
	basicLog(levelInfo, notATrace, false, "", "", a...)
}

// Infof prints a message if LOG_LEVEL is set to INFO or lower, with
// formatting.
func Infof(format string, a ...interface{}) {
	basicLog(levelInfo, notATrace, false, format, "", a...)
}

// Println prints a message if LOG_LEVEL is set to INFO or lower.
...
// Printf prints a message if LOG_LEVEL is set to INFO or lower, with
// formatting.
...
// Warn prints a message if LOG_LEVEL is set to WARN or lower.
func Warn(a ...interface{}) {
	basicLog(levelWarn, notATrace, false, "", "", a...)
}

// Warnf prints a message if LOG_LEVEL is set to WARN or lower, with
// formatting.
func Warnf(format string, a ...interface{}) {
	basicLog(levelWarn, notATrace, false, format, "", a...)
}

// Error prints a message if LOG_LEVEL is set to ERROR or lower.
func Error(a ...interface{}) {
	basicLog(levelErr, notATrace, false, "", "", a...)
}

// Errorf prints a message if LOG_LEVEL is set to ERROR or lower, with
// formatting.
func Errorf(format string, a ...interface{}) {
	basicLog(levelErr, notATrace, false, format, "", a...)
}

// Critical prints a message if LOG_LEVEL is set to CRITICAL or lower.
func Critical(a ...interface{}) {
	basicLog(levelCrit, notATrace, false, "", "", a...)
}

// Criticalf prints a message if LOG_LEVEL is set to CRITICAL or lower, with
// formatting.
