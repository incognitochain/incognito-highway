...
func TestKeepConnectionAtStart(t *testing.T) {
	hwAddrs := map[string][]rpcserver.HighwayAddr{"all": []rpcserver.HighwayAddr{rpcserver.HighwayAddr{Libp2pAddr: testHighwayAddress}}}
	...
	assert.Len(t, hmap.Peers[0], 2)
	assert.Len(t, hmap.Peers[4], 1)
}
